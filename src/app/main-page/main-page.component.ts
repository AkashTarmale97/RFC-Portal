...
export class MainPageComponent {
  ...
  onSubmit() {
    ...
    //         new Paragraph({
    //           children: [
    //             new TextRun({ text: 'PART 1: CHANGE REQUEST (To be completed by Change Requester)', bold: true, size: 24 }),
    //           ],
    //           spacing: { after: 300 },
    //         }),
    //         new Paragraph(`Title: ${this.formData.title}`),
    ...
    Packer.toBlob(wordDoc).then(blob => {
      saveAs(blob, "rfc-submission.docx");
