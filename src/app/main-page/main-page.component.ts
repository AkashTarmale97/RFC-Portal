...
export class MainPageComponent {
  Deployment_footprint: string | undefined;
  addFootprintRow() {
  ...
  formData = {
    ...
    tableData: [],
    Deployment_footprintlabel: '',
    pre_requisites: '',
    Total_activity_time: '',
    support_resource_requirement: '',
  ...
  deploymentTable: {
    Sr_no: string; API_Name: string; Broker: string; Remarks: string
  }[] = [];

  // Add row
  addRow() {
    this.deploymentTable.push({ Sr_no: '', API_Name: '', Broker: '', Remarks: '' });
  }

  // Remove row
  removeRow(index: number) {
    this.deploymentTable.splice(index, 1);
  }
  onSubmit() {
    ...
    const Description_of_Change_para = new Paragraph({
      children: [
        ...
        new TextRun({ break: 2 }),
        new TextRun("c) Common Component Change, if any:(Generic Library, DB Logger Library, Database Object definition, DPG, Cashe etc.): "),
        new TextRun({
    ...
    // --------------------------------------Deployment Footprint code---------------------------------------------------------------  

    const deploymentParagraphs = [];
    // Add label as a separate paragraph
    const DeploymentFootprintLabel = new Paragraph({
      spacing: {
        before: 100,
      },
      children: [
        new TextRun({
          text: "Deployment Footprint: if any:(Broker, EGs etc which were touched as part of deployment) ",
          bold: true,
          break: 2
        }),
        new TextRun({
          text: this.formData.Deployment_footprint,
          bold: true,
        }),
      ],
    });
    deploymentParagraphs.push(DeploymentFootprintLabel);

    if (this.formData.Deployment_footprint === 'Yes') {
      ...
      const DeploymentTable = new Table({
        rows: [
          new TableRow({
            children: ['Sr. No', 'API Name', 'Broker', 'Remarks'].map(
              text => new TableCell({ children: [new Paragraph(text)] })
            )
          }),
          ...this.deploymentTable.map(row =>
            new TableRow({
              children: [
                new TableCell({ children: [new Paragraph(row.Sr_no || '')] }),
                new TableCell({ children: [new Paragraph(row.API_Name || '')] }),
                new TableCell({ children: [new Paragraph(row.Broker || '')] }),
      ...
      deploymentParagraphs.push(DeploymentTable);

      // Optionally add space after the table
      deploymentParagraphs.push(
        new Paragraph({ children: [new TextRun({ break: 1 })] })
      );

      // // Add to your document
      // Paragraph.push(...deploymentParagraphs);
    }

    // -------------------------------=Total Activity Time=-----------------------------------------------------------------
    const splitTextToBullets = (text: string): Paragraph[] => {
      const points = (text || "")
        .split(/[\n;\r]|(?<=[.])\s+(?=[A-Z])/)
        .map(t => t.trim())
        .filter(Boolean);
    ...
    const prerequisiteParagraphs: Paragraph[] = [
      new Paragraph({
        children: [
          new TextRun({
            text:
              "Total Activity Time and Downtime, if any [This should include total activity duration considering the activity overlapping with business hours and its impact]",
            bold: true,
            break:1
          }),
        ],
      }),
      ...splitTextToBullets(this.formData.Total_activity_time),
    
      new Paragraph({
        children: [
          new TextRun({
            text:
              "Identified Dependencies [This should include the external and internal teams to be involved in the activity]",
            bold: true,
            break:1
          }),
        ],
      }),
      ...splitTextToBullets(this.formData.identified_dependencies),
    
      new Paragraph({
        children: [
          new TextRun({
            text:
              "Support Resource Requirements [This should include total resources required to perform the tasks]",
            bold: true,
            break:1
          }),
        ],
      }),
      ...splitTextToBullets(this.formData.support_resource_requirement),
    ];
